...
def _auprc_parallel_eval_phe_worker_init(
    big_data: np.ndarray,
...
def _auprc_parallel_eval_work(grp_key_indices_tuple):
    global _d
    global _metric
    ...
    all_treatment_prc_scores = [precision_score(y_true, s) for s in scores]
    results_dict = {
        "n_samples": len(scores),
        "trt_meanauprc": np.mean(all_treatment_prc_scores),
        "trt_auprcs": [all_treatment_prc_scores],
    }
...
def auprc(
    ...
    res_df = pd.DataFrame()
    if len(set(y)) == 1:
        print("Only one y target found, AUPRC requires multiple for calculation")
        return res_df
    if parallel:
        with mp.Pool(
            processes=None,
            initializer=_auprc_parallel_eval_phe_worker_init,
            initargs=(
                ds.data.values,
                phenotypic_metric,
                max_cardinality,
            ...
            for grp_name, res in tqdm(
                pool.imap_unordered(
                    _auprc_parallel_eval_work, grp_id_ilocs, chunksize=1
                ),
        ...
        for grp_key_indices_tuple in tqdm(grp_id_ilocs, desc="AUPRC", disable=quiet):
    ...
    return np.mean(res_df["trt_meanauprc"]) if len(res_df) > 0 else None, res_df
