...
class _DAVE_model(nn.Module):
    ...
    def _topol_to_inout_tuples(self,input:np.ndarray)->tuple[tuple[int, ...], tuple[int, int], tuple[tuple[int, ...]]]:
...
class DAVE:
    ...
    def fit(self, X, y):
        if self.model.topology[0][0] != X.shape[1] or self.model.topology[1][0] != y.shape[1]:
            self.model = _DAVE_model(view_sizes=(X.shape[1], y.shape[1]),n_hidden=self.num_hidden_layers).to(self.device)
        print(f"Constructed DAVE model on {self.device}, {self.batch_size=}, {self.learning_rate=}, {self.epochs=}, {self.num_hidden_layers=}")
        ...
        for epoch in range(self.epochs):  # loop over the dataset multiple times
            ...
            for i, data in enumerate(train_loader):
                ...
                loss = self.loss_function(reconA, reconB, X_train, y_train, muA, muB, logvarA, logvarB)
                loss.backward()
